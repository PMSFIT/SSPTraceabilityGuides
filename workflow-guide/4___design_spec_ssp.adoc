...
.	Exchange of interconnected simulation models and their parameterization
.	Support of the design by specifying empty components through their interface templates
...
However, more detailed information, such as design specifications or parts of the implementation and test environments, is often specified. A simple example is provided in the chapter examples (Figure. 13 and SSP-Trace-DesignSpec.ssp), the electromagnetic part of the DC motor is transferred as an FMU, while only the shell description with interfaces is provided for the mechanical part. It makes sense to use LS SSP-Traceability directly as a container for the artifacts here. The FMUs whose interconnection is specified as SSD are linked as inputs to the implementation phase (Figure 11). This approach makes it possible to trace what was input and what was created in the simulation task itself.
